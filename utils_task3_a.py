...
def evaluate(model, loader, loss_fn, device,test=False):
    model.eval()
    val_loss=0.0
    all_preds = []
    all_targets = []
    ...
    return {
        ...
        "confusion_matrix": cm if test else None,
        "y_pred": all_preds if test else None,
        "y_true": all_targets if test else None
    }
...
class FlatGCNClassifier(torch.nn.Module):
...
class FlatSAGEConvClassifier(torch.nn.Module):
...
class FlatGATClassifier(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, dropout=0.2):
        super().__init__()
